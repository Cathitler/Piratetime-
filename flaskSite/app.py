...
MENTORING_PASSWORD = "615925885"  # Password for the mentoring page
...
def load_analytics():
    if not os.path.exists(ANALYTICS_FILE):
        return {
            ...
            "session_tokens": {}  # Track sessions to prevent double counting
        }
...
def track_user(request_obj):
    ...
    if existing_user is None:
        ...
        # Only increment visit count if more than 5 minutes have passed since last visit
        last_visit = datetime.fromisoformat(existing_user["last_visit"])
        if (now - last_visit).total_seconds() > 300:  # 5 minutes
            existing_user["visit_count"] = existing_user.get("visit_count", 0) + 1
        existing_user["last_visit"] = now.isoformat()
        # Update user agent if changed
        if existing_user.get("user_agent") != user_agent:
            existing_user["user_agent"] = user_agent
    
    data["unique_users"] = unique_users
    
    # === TOTAL VISITS ===
    # Only increment total visits if more than 30 seconds have passed for this user
    session_key = f"{ip}_{user_agent[:30]}"
    session_data = data.get("session_tokens", {})
    
    if session_key not in session_data or (now_timestamp - session_data.get(session_key, 0)) > 30:
        data["total_visits"] = data.get("total_visits", 0) + 1
        session_data[session_key] = now_timestamp
    
    data["session_tokens"] = session_data
    ...
    active_users = [u for u in active_users if u.get("timestamp", 0) > cutoff]
    
    # Add/update current user
    existing_active = next((u for u in active_users if u["ip"] == ip), None)
    if existing_active:
        existing_active["timestamp"] = now_timestamp
        existing_active["user_agent"] = user_agent
    else:
        active_users.append({
            "ip": ip,
            "user_agent": user_agent,
...
# ===== Mentoring authorization =====
def mentoring_required(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        if not session.get('mentoring_logged_in'):
            return redirect(url_for('mentoring_login'))
        return f(*args, **kwargs)
    return decorated_function
...
# ===== Mentoring Routes (Password Protected) =====
@app.route("/mentoring/login", methods=["GET", "POST"])
def mentoring_login():
    if request.method == "POST":
        if request.form.get("password") == MENTORING_PASSWORD:
            session['mentoring_logged_in'] = True
            return redirect(url_for('mentoring_page'))
        return render_template("mentoring_login.html", error="Invalid password")
    return render_template("mentoring_login.html")

@app.route("/mentoring/logout")
def mentoring_logout():
    session.pop('mentoring_logged_in', None)
    return redirect(url_for('mentoring_login'))
...
def track_all_requests():
    # Skip tracking for static files and analytics endpoints
    if request.path.startswith('/static') or request.path.startswith('/thumbnails') or request.path.startswith('/videos'):
        return
    
    # Skip tracking for API endpoints that might cause recursion
    if request.path in ['/api/track', '/api/analytics']:
        return
...
def api_analytics():
    ...
    return jsonify({
        ...
        "active_users_count": len(data.get("active_users", [])),
        "unique_users": data.get("unique_users", []),
        "active_users": data.get("active_users", []),
